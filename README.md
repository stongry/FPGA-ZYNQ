...
承接 v5 87.94% 预 crop 精度，本阶段目标是**整个系统端到端**：任意一张 1280×720 场景图直接出车牌 + 行人，全部在板子上完成，不借用外部设备。

### 🔧 系统能力

| 任务 | 输入 | 输出 | 硬件 |
|------|------|------|------|
| 车牌识别 | 任意场景图 | 候选框 + 文字 | PL `plate_cnn_hls` |
| 行人检测 | 任意场景图 | 检测框 + 数量 | PL `pedcnn_hls_ip` 滑窗 |
| 边缘定位 | 320×240..1280×720 灰度 | 候选框 | PS Sobel + 网格密度 |

### 架构 (TCP 5004 "ALL\\0" 协议)

```
Client: 灰度或 RGB 图 → TCP 5004
                ↓
Board PS (ARM A53 裸机):
  ① RGB→Gray + RGB→HSV 蓝色掩码 (可选, fmt=1)
  ② Sobel 边缘检测 (PS 软件 ~100ms)
  ③ 网格密度扫描 → 候选矩形
  ④ 可选: HSV 掩码候选 (补充)
  ⑤ 跨源 NMS 去重 (IoU > 0.5)
  ⑥ 每个唯一候选发 3 variants (tight / +3px / +6px)
  ⑦ 像素精度 Y 轴 bbox 细化
                ↓
Board PL:
  ⑧ plate_cnn_hls (INT8) 每 crop ~675ms
  ⑨ pedcnn_hls_ip 每滑窗 ~42ms
                ↓
Board PS:
  ⑩ 行人 NMS (IoU > 0.3)
  ⑪ RES\\0 + 车牌 + 行人 序列化
...
### FPGA 资源占用 (v18 bitstream)

| IP | BRAM18 | DSP | 描述 |
|---|---|---|---|
| `plate_cnn_hls` | ~308 | 66 | 4 Conv + FC + 7 heads |
| `pedcnn_hls_ip` | ~20 | ~20 | 3 Conv + GAP + FC |
| AXI Smartconnect | ~5 | - | PS-PL 互联 |
| **总计** | **~330 / 432 (76%)** | **~86 / 360 (24%)** | ZU3EG |

### 🎯 端到端准确率 (10 张 CCPD 场景图)

| 配置 | 分辨率 | Exact match | 说明 |
|------|--------|-------------|------|
| 基线 (Sobel + 2 variants, PS CNN) | 720px | **5/10 = 50%** | 单源定位 |
| 加 NMS 去重 + 3 variants | 720px | 5/10 = 50% | 候选不重叠 |
| 加 HSV 颜色过滤 (RGB 协议) | 720px | 5/10 = 50% | Sobel+HSV 互补 |

**关键发现**：失败集合稳定为 #2, #7, #9 (Sobel/HSV 都给不出干净 crop) + #8/#10 (CNN 字符级误读 V→1, D→J)。**50% 是结构性天花板**，5 张失败中 4 张是 CNN 字符识别精度问题，不是定位问题。

### ✅ 已实现的 On-Board 功能

1. **TCP 5004 "ALL\\0" 全场景服务器** — 一次请求完成车牌 + 行人
2. **PS Sobel 边缘检测** — 3×3 算子，~100ms for 720×1160
3. **网格密度候选生成** — 8×8 cell，长宽比 2:1-7:1，Y 轴精化
4. **PS RGB→HSV 转换 + 蓝色掩码** — OpenCV 兼容 HSV (H 100-124, S>=43, V>=46)
5. **NMS 跨源候选去重** — Sobel + HSV 合并, IoU > 0.5
6. **Multi-variant crop 生成** — 每候选 3 variants (tight / +3 / +6)
7. **行人滑窗 + Sobel 密度预筛** — step=24 on 320×240, ~6s/帧
8. **行人 NMS (板上)** — IoU > 0.3

### 🔬 技术深探与关键经验

#### 1. lwIP TCP_WND=2048 的影响
- 接收窗口仅 2KB，835KB 原分辨率图传输需数分钟
- 客户端 MAX_DIM=720 降采样为推荐配置
- 如需全分辨率，需要增大 TCP_WND 并重建固件

#### 2. PL CNN 在 lwIP 上下文里不能直接用
- PL plate_cnn ~675ms/次，30 个候选 = 20s
- 长 busy-wait 锁死 GEM3 Ethernet IRQ 响应 → MAC 死锁
- **已发现但未集成的 workaround**：CNN 调用封顶 6 次（可测试），或改为异步提交
- 当前默认仍用 PS CNN，PL CNN 通过 UART 'V' 键切换

#### 3. 协议扩展
- `fmt=0`: 灰度 w*h 字节（兼容原协议）
- `fmt=1`: RGB888 3*w*h 字节（启用 HSV 定位）

#### 4. JTAG 部署要点
- `psu_init` → `psu_ps_pl_reset_config` → `psu_post_config` → `psu_ps_pl_isolation_removal` → `fpga` → `dow` → `con` 顺序不可错
- 如 con 后 CPU 卡在 0x100000：再执行一次 `stop` + `con` 手动唤醒

### 🛠 固件新增函数 (firmware/phase2b_main.c)

| 函数 | 作用 |
|------|------|
| `sobel_ps()` | 3×3 Sobel 边缘检测 (PS 软件) |
| `rgb_to_gray()` | RGB888 → 灰度 (BT.601 luma, 整数近似) |
| `rgb_to_hsv_blue_mask()` | RGB → 蓝色车牌 HSV 掩码 (OpenCV 兼容范围) |
| `find_plate_rects()` | 网格密度 → NMS → 3-variant 输出 |
| `refine_bbox_vertical()` | Y 轴像素精度 bbox 细化 |
| `crop_resize_128x32()` | 双线性 crop+resize 到 CNN 输入 |
| `pedcnn_run_patch()` | 单 64×128 窗 → PL pedcnn |
| `pedcnn_sliding()` | step=24 滑窗 + Sobel 密度预筛 + NMS |
| `downsample_320x240()` | 全图 → pedcnn 输入 |
| `all_run_and_reply()` | TCP 5004 编排器 |

### 📡 TCP 协议

**端口 5004 — 全场景多任务**
```
REQ: "ALL\0" + w(4) + h(4) + fmt(4) + (w*h 或 3*w*h) 字节
     fmt=0: 灰度;  fmt=1: RGB888
...
| 路径 | 工作量 | 预期增益 | 状态 |
|------|--------|---------|------|
| **异步 PL CNN**（lwIP 不阻塞）| 1-2 天 | 50% → 65% | 已识别根因，未实现 |
| **CCPD2019 + Focal Loss + crop-jitter 重训 PS CNN** | ~6 min 训练 | 59.53% on 8000-sample holdout | 已训练 (v6)，未部署 |
| **多尺度图像金字塔**（客户端同图传 2-3 scale）| 1-2 小时 | 50% → 65% | 未测 |
| **PL CNN 重训 + bitstream 重建** | 1-2 天 | 50% → 75%+ | 未尝试 |
| 单 HSV 颜色通道（不加 Sobel）| — | 80-92% 定位（文献）| 未隔离测 |

### 💻 演示输出

- `bitstreams/design_1_wrapper_v18_plate_pedcnn.bit` — 生产 bitstream (plate + pedcnn)
- Penn-Fudan 单行人场景: NMS 后 1 个 bbox ✅
- CCPD 街景: 每张 6-30 个候选，其中 1-3 个命中真值
