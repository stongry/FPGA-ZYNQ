...
#### 6 种 FPGA 可部署算法全面对比 (板上实测)

所有线性算法 (Template/SVM/LogReg) 部署在同一 Matmul HLS IP 上，仅更换权重矩阵。

| 排名 | 算法 | 准确率 | 运行位置 | 延迟 | 吞吐 | LUT% | FF% | BRAM% | DSP% | 功耗 | 能耗/次 | 权重大小 |
|:----:|------|:------:|:-------:|:----:|:----:|:----:|:---:|:-----:|:----:|:----:|:------:|:-------:|
| 1 | **CNN TinyLeNet** | **98.79%** | PL HLS | 4.9ms | 206/s | 12.8 | 5.2 | 12.2 | **22.5** | 80mW | 0.39mJ | 20KB BRAM |
| 2 | **MLP 784→64→10** | **95.20%** | PS A53 | 3.9ms | 258/s | 0 | 0 | 0 | 0 | 2300mW* | 9.0mJ | 200KB DDR |
| 3 | **Matmul 784→10** | **92.02%** | PL HLS | 0.7ms | 1400/s | 0.7 | 0.2 | 0.7 | 1.1 | 5mW | 0.004mJ | 8KB BRAM |
| 4 | **LogReg 784→10** | **89.43%** | PL HLS† | 0.7ms | 1400/s | 0.7 | 0.2 | 0.7 | 1.1 | 5mW | 0.004mJ | 8KB BRAM |
| 5 | **Linear SVM** | **81.65%** | PL HLS† | 0.7ms | 1400/s | 0.7 | 0.2 | 0.7 | 1.1 | 5mW | 0.004mJ | 8KB BRAM |
| 6 | **Template Match** | **63.10%** | PL HLS† | 0.7ms | 1400/s | 0.7 | 0.2 | 0.7 | 1.1 | 5mW | 0.004mJ | 8KB BRAM |

> *PS 功耗 2.3W 为 ARM+DDR 子系统共享
> † 复用 Matmul HLS 硬件 (784 input × 10 output INT8 矩阵乘)，仅更换权重

#### 不可部署算法参考 (Python/x86 离线评估)

| 算法 | 准确率 | 不可部署原因 |
|------|:------:|------------|
| Random Forest (100棵) | 93.21% | 树遍历分支逻辑，BRAM 无法存储 100 棵决策树 |
| KNN (k=3) | 91.80% | 需存储全部训练样本 (47MB)，远超 BRAM 容量 |
| Naive Bayes | 58.01% | 需 log/exp 概率运算，精度太低不值得部署 |
...
★ CNN:     最高精度(98.79%)，中等速度，最多资源
● MLP:     高精度(95.2%)，零PL资源，高功耗
○ Matmul:  均衡(92%)，最快(0.7ms)，最省(5mW)
□◇△:       低精度但与Matmul共享硬件，零额外成本
```

#### 关键结论

1. **精度优先** → CNN TinyLeNet (PL)：98.79%，碾压所有算法，80mW 低功耗
2. **速度优先** → Matmul INT8 (PL)：0.7ms / 1400 img/s，仅 1% 资源
3. **零 PL 资源** → MLP float32 (PS)：95.2%，纯软件灵活可改
4. **零额外成本** → LogReg/SVM/Template 复用 Matmul 硬件换权重即可
5. **能效比**: PL CNN 每次推理 0.39mJ vs PS MLP 9.0mJ → **PL 省电 23 倍**
