...
#### 9 种算法超级对比表 (FPGA 板上实测, MNIST 10000 张)

| | **CNN TinyLeNet** | **MLP 2层** | **Matmul 单层** | **LogReg** | **Linear SVM** | **Template** | Random Forest | KNN (k=3) | Naive Bayes |
|--|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **网络结构** | Conv×2+Pool×2+FC×2 | 784→64→10 | 784→10 | 784→10+softmax | 784→10 最大间隔 | 10模板相关 | 100棵决策树 | 距离投票 | 概率贝叶斯 |
| **运行位置** | **PL HLS** | **PS ARM A53** | **PL HLS** | **PL HLS†** | **PL HLS†** | **PL HLS†** | ⚠️不可部署 | ⚠️不可部署 | ⚠️不可部署 |
| **部署状态** | ✅ 实测 | ✅ 实测 | ✅ 实测 | ✅ 可部署† | ✅ 可部署† | ✅ 可部署† | ❌ | ❌ | ❌ |
| ─ **精度** ─ | | | | | | | | | |
| **准确率** | **98.79%** | 95.20% | 92.02% | 89.43% | 81.65% | 63.10% | 93.21% | 91.80% | 58.01% |
| digit 0 | 99.8% | ~99% | ~97% | — | — | — | — | — | — |
| digit 5 (最弱) | 97.5% | ~92% | ~87% | — | — | — | — | — | — |
| 量化方式 | INT8 | float32 | INT8 | INT8 | INT8 | INT8 | — | — | — |
| 量化损失 | 0.04% | 无 | ~1% | ~1% | ~1% | ~1% | — | — | — |
| ─ **速度** ─ | | | | | | | | | |
| **单张延迟** | 4.9ms | 3.9ms | **0.7ms** | **0.7ms** | **0.7ms** | **0.7ms** | — | 5.3ms* | — |
| **吞吐量** | 206/s | 258/s | **1,400/s** | **1,400/s** | **1,400/s** | **1,400/s** | — | — | — |
| 10K 总耗时 | 48.5s | 38.8s | **7.1s** | **7.1s** | **7.1s** | **7.1s** | — | ~53s* | — |
| TCP 开销 | 0.7ms | 0.7ms | 0.7ms | 0.7ms | 0.7ms | 0.7ms | — | — | — |
| 纯计算时间 | 3.5ms | 2.5ms | 0.02ms | 0.02ms | 0.02ms | 0.02ms | — | — | — |
| ─ **资源** ─ | | | | | | | | | |
| **LUT** | 9,000 (12.8%) | 0 | 500 (0.7%) | 500† | 500† | 500† | 数千 | 不可行 | — |
| **FF** | 7,400 (5.2%) | 0 | 300 (0.2%) | 300† | 300† | 300† | — | — | — |
| **BRAM18** | 35 (12.2%) | 0 | 2 (0.7%) | 2† | 2† | 2† | ~100+ | 全部 | — |
| **DSP48** | **81 (22.5%)** | 0 | 4 (1.1%) | 4† | 4† | 4† | 0 | 0 | — |
| ─ **功耗** ─ | | | | | | | | | |
| **PL 动态功耗** | 80mW | 0 | 5mW | 5mW | 5mW | 5mW | — | — | — |
| **PS 功耗** | — | 2,300mW | — | — | — | — | — | — | — |
| **每次推理能耗** | 0.39mJ | 9.0mJ | **0.004mJ** | **0.004mJ** | **0.004mJ** | **0.004mJ** | — | — | — |
| **vs PS MLP 省电** | 23x | 1x (基准) | **2,250x** | **2,250x** | **2,250x** | **2,250x** | — | — | — |
| ─ **存储** ─ | | | | | | | | | |
| 权重数量 | 20,424 | 50,826 | 7,840 | 7,840 | 7,840 | 7,840 | ~百万节点 | 47M 样本 | 15,680 |
| 权重大小 | 20KB | 200KB | 8KB | 8KB | 8KB | 8KB | ~5MB | 47MB | 64KB |
| 存储位置 | PL BRAM | PS DDR | PL BRAM | PL BRAM | PL BRAM | PL BRAM | — | — | — |
| 固件增量 | +70KB | +202KB | +18KB | +8KB | +8KB | +8KB | — | — | — |
| ─ **扩展性** ─ | | | | | | | | | |
| 可换权重热更新 | 需重建bit | ✅ 改DDR | 需重建bit | ✅ 复用HW | ✅ 复用HW | ✅ 复用HW | — | — | — |
| 多模型并行 | 需额外IP | ✅ 软件切换 | 与LogReg/SVM共存 | 共用Matmul | 共用Matmul | 共用Matmul | — | — | — |
| 迁移到其他任务 | 需重训练+综合 | 需重训练 | 需重训练 | 需重训练 | 需重训练 | 仅需新模板 | — | — | — |
| 不可部署原因 | — | — | — | — | — | — | 树结构不适合PL | 47MB超BRAM | 精度58%不值得 |
| **推荐场景** | 高精度 | 灵活原型 | 低功耗+高速 | 低成本扩展 | 低成本扩展 | 最简基线 | — | — | — |

> † 复用 Matmul HLS 硬件 (784×10 INT8 矩阵乘)，零额外 PL 资源，仅更换权重
> \* KNN 时间为 Python/x86 参考值，非 FPGA 实测

**关键结论:**
1. **CNN (PL) 98.79%** 碾压全部，功耗仅 80mW，精度比最强传统算法 (RF 93.2%) 高 5.6%
2. **Matmul (PL) 0.7ms** 最快，能效比 PS MLP **高 2250 倍**，仅占 1% FPGA 资源
3. **线性方法** (LogReg/SVM/Template) **零额外成本**复用 Matmul 硬件，适合快速实验
4. **PS MLP** 唯一零 PL 资源方案，95.2% 精度，但功耗 2.3W 是 CNN 的 29 倍
...
### 9 种行人检测算法超级对比表 (Penn-Fudan 数据集, 300 正+300 负, 5-fold CV)

| | **HOG+AdaBoost** | **HOG+RF** | **HOG+SVM** | **HOG+LogReg** | **Pixel+NB** | **LBP+SVM** | **HOG+KNN** | **Pixel+SVM** | **Template** |
|--|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **特征提取** | HOG 3780D | HOG 3780D | HOG 3780D | HOG 3780D | Raw 8192D | LBP 256D | HOG 3780D | Raw 8192D | 相关匹配 |
| **分类器** | AdaBoost(50) | RF(50棵) | Linear SVM | Logistic Reg | Gaussian NB | Linear SVM | KNN k=3 | Linear SVM | 阈值 |
| **运行位置** | ⚠️PS only | ⚠️PS only | **✅ PL HLS** | PL 可复用† | ⚠️PS only | PL 可部署‡ | ⚠️不可部署 | PL 可复用† | PL 可复用† |
| **部署状态** | ⚠️仅PS | ⚠️仅PS | **✅ 已部署实测** | ✅ 可部署† | ⚠️仅PS | ✅ 需新IP‡ | ❌ | ✅ 可部署† | ✅ 可部署† |
| ─ **精度** ─ | | | | | | | | | |
| **CV 准确率** | **96.2%** | **94.7%** | **89.3%** | 89.5% | 89.2% | 69.5% | 66.2% | 64.5% | 50.0% |
| 训练准确率 | 100% | 100% | 100% | 100% | 90.3% | 69.5% | 100% | 94.3% | 50.0% |
| 召回率 (板上实测) | — | — | **100%** | ~90%† | — | — | — | — | — |
| 精确率 (板上实测) | — | — | **90.9%** | — | — | — | — | — | — |
| F1 Score (板上实测) | — | — | **95.2%** | — | — | — | — | — | — |
| 过拟合风险 | 中 | 高 | 低 | 低 | 低 | 低 | 无 | 中 | 无 |
| ─ **速度** ─ | | | | | | | | | |
| **输入尺寸** | 320×240 | 320×240 | 320×240 | 320×240 | 64×128 | 64×128 | 320×240 | 64×128 | 64×128 |
| **滑窗数量** | 495 | 495 | 495 | 495 | 1 | 1 | 495 | 1 | 1 |
| **FPGA 单帧延迟** | ~50ms(PS) | ~50ms(PS) | **22ms(PL)** | **22ms(PL)†** | ~30ms(PS) | ~15ms(PL)‡ | 不可行 | **22ms(PL)†** | **22ms(PL)†** |
| **帧率** | ~20 FPS | ~20 FPS | **46 FPS** | **46 FPS†** | ~33 FPS | ~67 FPS‡ | — | **46 FPS†** | **46 FPS†** |
| 特征提取耗时 | ~15ms(PS) | ~15ms(PS) | **含在PL** | **含在PL** | 0 | ~5ms | ~15ms | 0 | 0 |
| 分类器耗时 | ~35ms(PS) | ~35ms(PS) | **含在PL** | **含在PL** | ~30ms | ~10ms | 秒级 | **含在PL** | **含在PL** |
| 数据传输方式 | DDR直接 | DDR直接 | **m_axi DMA** | m_axi DMA† | DDR直接 | AXI-Lite‡ | — | m_axi DMA† | m_axi DMA† |
| ─ **FPGA 资源** ─ | | | | | | | | | |
| **LUT** | 0(PS) | 0(PS) | 5,000 (7.1%) | 5,000† | 0(PS) | ~3,000‡ | 不可行 | 5,000† | 5,000† |
| **FF** | 0 | 0 | 5,000 (3.5%) | 5,000† | 0 | ~2,000‡ | — | 5,000† | 5,000† |
| **BRAM18** | 0 | 0 | **90 (31.3%)** | 90† | 0 | ~20‡ | 需全部 | 90† | 90† |
| **DSP48** | 0 | 0 | 4 (1.1%) | 4† | 0 | 2‡ | — | 4† | 4† |
| 额外 AXI 基础设施 | 0 | 0 | SmartConnect | 共用† | 0 | 需新连接‡ | — | 共用† | 共用† |
| ─ **功耗** ─ | | | | | | | | | |
| **PL 动态功耗** | 0 | 0 | **126mW** | 126mW† | 0 | ~60mW‡ | — | 126mW† | 126mW† |
| **PS 功耗** | 2,300mW | 2,300mW | 0 | 0 | 2,300mW | 0 | — | 0 | 0 |
| **总功耗** | 2,300mW | 2,300mW | **126mW** | 126mW† | 2,300mW | ~60mW‡ | — | 126mW† | 126mW† |
| **每帧能耗** | ~115mJ | ~115mJ | **2.77mJ** | 2.77mJ† | ~69mJ | ~0.9mJ‡ | — | 2.77mJ† | 2.77mJ† |
| vs PS HOG+SVM | 1x | 1x | **41x 省电** | 41x† | 1.7x | ~128x‡ | — | 41x† | 41x† |
| ─ **存储** ─ | | | | | | | | | |
| 模型参数量 | 50×弱分类器 | 50棵×深度 | 3,780 | 3,780 | 15,680 | 256 | 60K×3780 | 8,192 | 8,192 |
| 权重大小 | ~500KB | ~5MB | 8KB (INT8) | 8KB (INT8) | 64KB | 1KB (INT8) | 47MB+ | 8KB (INT8) | 8KB |
| 存储位置 | PS DDR | PS DDR | PL BRAM | PL BRAM | PS DDR | PL BRAM | 不可行 | PL BRAM | PL BRAM |
| 固件增量 | +500KB | +5MB | +18KB | +8KB(换权重) | +66KB | +12KB | — | +8KB | +8KB |
| ─ **扩展性** ─ | | | | | | | | | |
| 多尺度检测 | PS缩放重跑 | PS缩放重跑 | PS缩放重跑 | PS缩放重跑 | PS缩放 | PS缩放重跑 | — | PS缩放 | PS缩放 |
| NMS 后处理 | 需PS软件 | 需PS软件 | 需PS软件 | 需PS软件 | 需PS | 需PS软件 | — | 需PS | 需PS |
| 最大检测数 | 无限(PS) | 无限(PS) | 16(PL硬件) | 16† | 1 | 16‡ | — | 16† | 16† |
| 可换权重热更新 | 需重训练 | 需重训练 | ✅ 换权重 | ✅ 换权重 | 需重训练 | ✅ 换权重 | — | ✅ 换权重 | ✅ 换权重 |
| 可迁移到其他目标 | 需重训练 | 需重训练 | 需重训练 | 需重训练 | 需重训练 | 需重训练 | — | 需重训练 | 需新模板 |
| 与 CNN 共存 | ✅(PS) | ✅(PS) | ✅(独立IP) | ✅共用IP | ✅(PS) | 需额外IP | — | ✅共用IP | ✅共用IP |
| ─ **部署难度** ─ | | | | | | | | | |
| HLS 综合时间 | — | — | ~50s | 0(复用) | — | ~30s | — | 0(复用) | 0(复用) |
| Vivado 重建时间 | — | — | ~15min | 0(复用) | — | ~15min | — | 0(复用) | 0(复用) |
| 训练复杂度 | 中 | 低 | 低 | 低 | 极低 | 低 | 无 | 低 | 极低 |
| 不可部署原因 | Boost集成复杂 | 树结构不适合 | — | — | 需log/exp | — | 47MB超BRAM | — | — |
| **推荐场景** | 最高精度(离线) | 高精度(离线) | **实时部署首选** | 低成本替代 | 简单基线 | 极低功耗 | 不推荐 | 不推荐 | 不推荐 |

> † 复用 HOG+SVM HLS 硬件（梯度计算+HOG直方图+滑窗SVM 全在 PL），仅更换 SVM 权重矩阵
> ‡ LBP+SVM 需要单独 HLS IP（LBP 特征提取+线性分类器），资源为估算值
> PS 延迟为 ARM Cortex-A53 @ 1.2GHz 估算，含特征提取+分类器+TCP 开销

**关键结论:**
1. **精度**: HOG+AdaBoost(96.2%) > HOG+RF(94.7%) > HOG+SVM/LogReg(89%) > Pixel+NB(89%) > LBP(70%) > KNN(66%) > Pixel+SVM(65%) > Template(50%)
2. **速度**: LBP+SVM(67FPS) > HOG+SVM系列(46FPS) > Pixel+NB(33FPS) > PS方案(20FPS) — PL 比 PS 快 **2.3 倍**
3. **功耗**: LBP+SVM(60mW) < HOG+SVM系列(126mW) << PS方案(2300mW) — PL 比 PS 省电 **41 倍**
4. **资源**: PS方案(0%) < LBP(~4%) < HOG+SVM(7.1% LUT + 31.3% BRAM) — BRAM 是主要瓶颈
5. **实用性**: HOG+SVM (PL) 是精度/速度/功耗的**最佳平衡点**，唯一经过板上完整验证
6. **零成本扩展**: LogReg/Pixel+SVM/Template 可复用 HOG+SVM 硬件，仅换 8KB 权重
