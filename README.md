...
### 9 种行人检测算法超级对比表 (Penn-Fudan 数据集, 5-fold CV)
...
| 可换权重 | 需重训练 | 需重训练 | ✅换权重 | ✅换权重 | 需重训练 | ✅换权重 | — | ✅换权重 | ✅换权重 |
| 不可部署原因 | Boost集成复杂 | 树结构不适合 | — | — | 需log/exp | — | 47MB超BRAM | — | — |
| **推荐场景** | 最高精度(PS) | 高精度(PS) | **实时部署首选** | 低成本替代 | 简单基线 | 极低功耗 | 不推荐 | 不推荐 | 不推荐 |

> † 复用 HOG+SVM HLS 硬件，仅更换分类器权重
> ‡ LBP+SVM 需要单独 HLS IP（LBP 特征提取+线性分类器），资源为估算值
> PS 延迟为 ARM A53 @ 1.2GHz 估算，实际含 TCP 开销

**关键结论:**
1. **HOG+AdaBoost (96.2%)** 精度最高但只能跑在 PS 上，115mJ/帧
2. **HOG+SVM (PL HLS)** 是唯一已部署验证的实时方案：22ms/46FPS/126mW/2.77mJ
3. **LBP+SVM** 理论上资源最省(无 HOG 计算)，但精度仅 69.5%
4. **HOG+LogReg/Pixel+SVM** 可零成本复用 HOG+SVM HLS 硬件换权重
5. PL 加速比 PS 省电 **41 倍**（2.77mJ vs 115mJ）
